...
class ELMRegressor(BaseELM, RegressorMixin):
    ...
    #     return K
    #
    # def Kernel_function(self,x1,x2,gamma=1.0):
    #     return np.exp(-pow(x1-x2,2.0)/2*gamma**2)
    ...
    def _fit_regression(self, y):
        ...
        if (self.regressor is None):
            ...
            c = pinv2(self.hidden_activations_)
            self.c = c
            self.coefs_ = safe_sparse_dot(c,y)

            # self.coefs_ = lim.coef_.T
            # print self.coefs_.max()
            # print self.coefs_.min()
        else:
...
class ELMClassifier(BaseELM, ClassifierMixin):
    ...
    def save(self,npy_path):
        dicts = {}
        dicts['h_a'] = self.elm_regressor_.c
        dicts['beta'] = self.elm_regressor_.coefs_
        np.save(dicts,npy_path)
